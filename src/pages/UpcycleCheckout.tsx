...
const UpcycleCheckout: React.FC = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios
      .get(`/upcycle-requests/${id}`)
      .then((res) => {
        setRequest(res.data);
      })
      .catch(() => toast.error("Failed to fetch upcycle request"))
  ...
  const handlePayNow = async () => {
    if (!request) return;
    await axios
      .post(`/recycle-requests/${request.id}/pay`, {
  ...
  if (!request) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-red-500">Upcycle request not found.</div>
      </div>
    );
  }
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-gray-900">
                      Rp{request.total_pay.toLocaleString()}
                    </span>
                  </div>
                  {request.notes && (
...
export default UpcycleCheckout;
